//! vLLM local server provider (OpenAI-compatible endpoint).
//!
//! POST http://localhost:8000/v1/chat/completions

use crate::llm::config::ProviderType;
use crate::llm::LlmProvider;
use anyhow::Result;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, Serialize)]
struct VllmRequest {
    model: String,
    messages: Vec<VllmMessage>,
    max_tokens: u32,
    temperature: f64,
}

#[derive(Debug, Clone, Serialize)]
struct VllmMessage {
    role: String,
    content: String,
}

#[derive(Debug, Deserialize)]
struct VllmResponse {
    choices: Vec<VllmChoice>,
}

#[derive(Debug, Deserialize)]
struct VllmChoice {
    message: VllmChoiceMessage,
}

#[derive(Debug, Deserialize)]
struct VllmChoiceMessage {
    content: String,
}

#[derive(Debug, Deserialize)]
struct VllmError {
    error: VllmErrorDetail,
}

#[derive(Debug, Deserialize)]
struct VllmErrorDetail {
    message: String,
}

#[derive(Clone)]
pub struct VllmProvider {
    client: reqwest::Client,
    config: crate::llm::config::ProviderConfig,
}

impl VllmProvider {
    pub fn new(config: crate::llm::config::ProviderConfig) -> Self {
        Self {
            client: reqwest::Client::new(),
            config,
        }
    }

    async fn do_generate(&self, system: &str, prompt: &str) -> Result<String> {
        let body = VllmRequest {
            model: self.config.model.model.clone(),
            messages: vec![
                VllmMessage {
                    role: "system".to_string(),
                    content: system.to_string(),
                },
                VllmMessage {
                    role: "user".to_string(),
                    content: prompt.to_string(),
                },
            ],
            max_tokens: self.config.model.max_tokens,
            temperature: self.config.model.temperature,
        };

        let mut req = self.client
            .post(format!("{}/v1/chat/completions", self.config.base_url))
            .header("content-type", "application/json")
            .json(&body);

        // vLLM may or may not require auth depending on deployment
        if let Some(ref key) = self.config.api_key {
            req = req.bearer_auth(key);
        }

        let resp = req.send().await?;

        let status = resp.status();
        if !status.is_success() {
            let err_text = resp.text().await?;
            return Err(anyhow::anyhow!("vLLM API error ({}): {}", status, err_text));
        }

        let resp_body: VllmResponse = resp.json().await?;

        resp_body
            .choices
            .into_iter()
            .find_map(|c| if c.message.content.is_empty() { None } else { Some(c.message.content) })
            .ok_or_else(|| anyhow::anyhow!("vLLM returned empty content"))
    }
}

#[async_trait::async_trait]
impl LlmProvider for VllmProvider {
    fn provider_type(&self) -> ProviderType {
        ProviderType::Vllm
    }

    async fn generate(&self, system: &str, prompt: &str) -> Result<String> {
        self.do_generate(system, prompt).await
    }
}
